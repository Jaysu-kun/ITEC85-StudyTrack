    ...
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div
          ...
          className="absolute right-0 mt-2.5 w-80 sm:w-96 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 shadow-2xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200"
        >
          {/* Header */}
          <div className="p-4 bg-slate-50/80 dark:bg-slate-800/60 border-b border-slate-100 dark:border-slate-800/80 flex items-center justify-between">
            ...
            {totalAlertsCount > 0 && (
              <span className="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-amber-100 text-amber-800 dark:bg-amber-950/80 dark:text-amber-300 border border-amber-200 dark:border-amber-800/60">
                Action Needed
              </span>
            )}
          </div>
            ...
            {totalAlertsCount === 0 ? (
              ...
              <>
                ...
                {urgentTasks.length > 0 && (
                  <div className="p-2">
                    <div className="px-2 py-1 text-[11px] font-bold uppercase tracking-wider text-amber-600 dark:text-amber-400 flex items-center gap-1.5">
                      <Clock size={12} />
                    ...
                    <div className="space-y-1.5 mt-1">
                      {urgentTasks.map((task) => {
                        ...
                        return (
                          <div
                            key={task.id}
                            className="group p-2.5 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/70 border border-amber-200/60 dark:border-amber-900/40 bg-amber-50/30 dark:bg-amber-950/20 transition-all flex items-start justify-between gap-2.5 cursor-pointer"
                          ...
                          >
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-1.5">
                                ...
                                />
                                <h3 className="text-xs font-bold text-slate-900 dark:text-slate-100 truncate group-hover:text-sky-600 dark:group-hover:text-sky-400 transition-colors">
                                  {task.title}
                                </h3>
                              </div>
                            ...
                            <button
                              ...
                              className="p-1 rounded-lg text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-950/50 transition-colors flex-shrink-0"
                            >
                              <CheckCircle size={16} />
                            </button>
                          </div>
                ...
                {overdueTasks.length > 0 && (
                  <div className="p-2">
                    <div className="px-2 py-1 text-[11px] font-bold uppercase tracking-wider text-rose-600 dark:text-rose-400 flex items-center gap-1.5">
                    ...
                    <div className="space-y-1.5 mt-1">
                      {overdueTasks.map((task) => (
                        <div
                          key={task.id}
                          className="group p-2.5 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/70 border border-rose-200/60 dark:border-rose-900/40 bg-rose-50/30 dark:bg-rose-950/20 transition-all flex items-start justify-between gap-2.5 cursor-pointer"
                          onClick={() => handleTaskClick(task)}
                        >
                          <div className="flex-1 min-w-0">
                            <h3 className="text-xs font-bold text-slate-900 dark:text-slate-100 truncate group-hover:text-rose-600 transition-colors">
                              {task.title}
                            </h3>
                            <div className="mt-1 flex items-center gap-2 text-[11px] text-rose-600 dark:text-rose-400 font-semibold">
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <button
                    ...
                    className="mt-1.5 inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-[11px] font-bold bg-sky-600 hover:bg-sky-700 text-white transition-colors shadow-sm"
                  >
