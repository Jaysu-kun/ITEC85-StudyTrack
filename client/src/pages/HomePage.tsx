...
export const HomePage: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-16">
      ...
      {/* Hero Welcome Header */}
      <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-5 bg-gradient-to-r from-sky-500/10 via-blue-500/5 to-transparent p-6 sm:p-8 rounded-3xl border border-sky-200/70 dark:border-sky-900/40 backdrop-blur-md shadow-sm">
        <div>
          {/* Subtle Section Label */}
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-sky-100/90 text-sky-800 dark:bg-sky-950/80 dark:text-sky-300 border border-sky-200/80 dark:border-sky-800/60 mb-2.5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5 sm:gap-4 mb-8">
        {[
          ...
          <Card key={m.label} variant="default" className="p-4 sm:p-5 flex items-center gap-3.5 hover:shadow-md transition-shadow">
            <div className={`p-2.5 rounded-xl border ${m.badge}`}>{m.icon}</div>
            <div>
              <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">{m.label}</p>
              <p className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white mt-0.5">{m.value}</p>
            </div>
          </Card>
        ))}
      </div>
          ...
          <AnimatePresence>
            {isAddingTask && (
              <motion.div
                ...
                className="mb-8"
              >
                <Card variant="glass" className="p-6 sm:p-8 border-2 border-sky-500/80 dark:border-sky-500/60">
                  <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-100 dark:border-slate-800">
                    <h2 className="text-lg sm:text-xl font-bold text-slate-900 dark:text-white">
                      Create New Task
                    </h2>
                    <span className="text-xs px-3 py-1 rounded-full bg-sky-100 dark:bg-sky-950/80 text-sky-700 dark:text-sky-300 font-semibold border border-sky-200/80 dark:border-sky-800/60">
                      New Task
                    </span>
                  </div>
                  <TaskForm
            ...
            <div className="flex flex-col md:flex-row gap-3.5 items-stretch md:items-center justify-between">
              ...
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-4 py-2 text-sm rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-sky-500/25 focus:border-sky-500 transition-all"
                />
              </div>

              {/* Status and Priority Controls */}
              <div className="flex flex-wrap items-center gap-2">
                <fieldset className="p-1 m-0 min-w-0 flex items-center gap-1 bg-slate-100 dark:bg-slate-800/90 rounded-xl border border-slate-200/60 dark:border-slate-700/60">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="py-1.5 px-3 rounded-xl text-xs font-semibold border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-sky-500/25 focus:border-sky-500"
                  >
